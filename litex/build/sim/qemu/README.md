...
LiteX `main_ram` slave through the `qemu_shared_ram` simulation module.
...
The shared RAM module still uses a simple 32-bit single-word access path to the
mapped backing file internally, but exposes the selected SoC bus standard to the
LiteX interconnect. It shares storage, not cache management: software that
...
For `wishbone`, the socket module drives the Wishbone master directly. For
`axi-lite` and `axi`, the QEMU CPU wrapper exposes an AXI-Lite/AXI master and
converts the internal single-beat request path locally. The supported SoC bus
standards are `wishbone`, `axi-lite`, and `axi`. The helper below elaborates
the QEMU CPU against the RV32/RV64 matrix without launching QEMU or compiling
generated software/gateware:

```sh
python3 litex/build/sim/qemu/check_qemu_bus_matrix.py --with-shared-ram
```

Full AXI burst/ID traffic is not generated by the v1 bridge protocol yet; AXI
support currently issues single-beat AXI transactions derived from the QEMU
request stream.
