...
LiteX `main_ram` slave through the bus-native QEMU shared RAM simulation
module.
...
Use `--qemu-no-shared-ram` to return to the older model where LiteX integrated
main RAM is instantiated in Verilator and QEMU owns its separate RAM:
...
The shared RAM modules still use a simple 32-bit single-word access path to the
mapped backing file internally, but expose the selected SoC bus standard to the
LiteX interconnect: `qemu_shared_ram`, `qemu_axi_lite_shared_ram` or
`qemu_axi_shared_ram`. They share storage, not cache management: software that
mixes CPU caches and peripheral DMA still needs the normal cache
maintenance/uncached mapping discipline.

## Bus Standards

The working bridge protocol is single-beat MMIO. The QEMU CPU wrapper exposes a
LiteX master matching the selected `--bus-standard`:
...
The bridge module is selected from `--bus-standard`:

- `wishbone`: `qemu_wishbone`
- `axi-lite`: `qemu_axi_lite`
- `axi`: `qemu_axi`

The supported SoC bus standards are `wishbone`, `axi-lite`, and `axi`. The
helper below elaborates the QEMU CPU against the RV32/RV64 matrix without
launching QEMU or compiling generated software/gateware:

```sh
python3 litex/build/sim/qemu/check_qemu_bus_matrix.py --with-shared-ram
```

Full AXI burst traffic is not generated by the v1 QEMU MMIO protocol yet. The
native AXI module drives channel-level AXI directly, but each QEMU access is
issued as a blocking single-beat transaction with ID 0.
