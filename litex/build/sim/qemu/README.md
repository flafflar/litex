...
The current shared RAM module is a simple 32-bit Wishbone slave with
single-cycle, single-word accesses. It shares storage, not cache management:
software that mixes CPU caches and peripheral DMA still needs the normal cache
maintenance/uncached mapping discipline.

## Bus Standards

The working bridge protocol is single-beat MMIO and the Verilator-side module
currently presents a Wishbone master to LiteX. LiteX can then adapt that master
to the selected SoC bus with the normal `--bus-standard` option:
...
support is currently through LiteX's existing Wishbone-to-AXI conversion.
