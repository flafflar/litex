...
def apply_patch(args):
    ...
    if check.returncode == 0:
        run(["git", "apply", args.patch], cwd=args.src_dir)
        return
...
def ensure_minimal_device_configs(args):
    if not args.minimal_devices:
        return
    ...
    for target in targets:
        ...
        if not arch.startswith("riscv"):
            continue
        config_path = args.src_dir / "configs" / "devices" / target / "litex-sim.mak"
        if not config_path.exists():
            config_path.write_text(config, encoding="utf-8")
            print("Wrote {}".format(config_path), flush=True)
...
def main():
    ...
    ensure_source(args)
    apply_patch(args)
    ensure_minimal_device_configs(args)
    build_qemu(args)
